...
#define TEN_MINUTES 3982447 // base: 3982447
#define K 250
...
void Train_and_Load_Model(int training){
    int model_created=0;
    FILE *flows;
    if(training)
        flows = fopen("/home/takatsukiaa/ML-Sketch/Python/TopK/training_flows.csv","w+");
    else
        flows = fopen("/home/takatsukiaa/ML-Sketch/Python/TopK/testing_flows.csv","w+");
    uint num_of_flows = 0;
    ...
    for (auto it = actual_size.begin(); it != actual_size.end(); ++it) {
        ...
        string flow_id = ToHex(it->first);
        Flow temp_flow = { flow_id, (float)it->second};
        
        auto it_actual = find(actualSet.begin(), actualSet.end(), temp_flow);
        if (it_actual != actualSet.end()){
            fprintf(flows,"1,");
            num_of_flows++;
        }
            ...
            fprintf(flows,"0,");   
        TestSketch->PrintCounterFile(temp, second, flows);
    }
    fclose(flows);
    printf("First 10 minutes contained %lu flows\n", actual_size.size());
    // int flag = 1;
